...
from lsy_drone_racing.control.ocp_solver_old import create_ocp_solver
...
class PmmMPC(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        self._rotor_vel = 0.0*np.ones(4)  # initial rotor_vel  
        ...
        self.last_u = np.array([0.0, 0.0, 0.0, hover_thrust, 1.0])
        ...
        self._generate_gate_waypoints(
            self._pos + np.array([0.0, 0.0, 0.3]), self._current_gate_idx, self._distance_before, self._distance_after
        )
    ...
    def compute_control(
        ...
        x0 = np.concatenate((self._pos, self._rpy, self._vel, self._drpy, self._rotor_vel, np.array([theta0])))
        self._acados_ocp_solver.set(0, "lbx", x0)
        self._acados_ocp_solver.set(0, "ubx", x0)

        # Build MPCC parameter horizon & warm-start states
        p_horizon = self._build_mpcc_parameters(theta0)
        x_guess = self._build_x_guess(theta0)
        ...
        self._last_theta = float(x1[-1])
        u1 = self._acados_ocp_solver.get(1, "u")
        self._last_vtheta = float(u1[-1])
        vtheta0 = self._last_vtheta
        self._rotor_vel = x1[12:16]  # index of rotor_vel in state
    ...
    def _build_mpcc_parameters(self, theta0: float) -> NDArray[np.floating]:
        """
    ...
    def _build_x_guess(self, theta0: float) -> NDArray[np.floating]:
        """
    ...
    def _apply_constraints(
        ...
        for j in range(self._N):
            ...
            # warm-start state
            #self._acados_ocp_solver.set(stage, "x", x_guess)
    ...
    def _project_on_pmm_path(self, pos: NDArray[np.floating]) -> tuple[float, int]:
        ...
        idx_raw = int(np.argmin(dists))
        
        # Init last index if needed
        if not hasattr(self, "_last_proj_idx"):
            self._last_proj_idx = idx_raw

        # Rücksprünge verbieten: index nicht kleiner als zuletzt
        idx = max(idx_raw, self._last_proj_idx)
        # optional: vorwärts-Sprung begrenzen für Stabilität
        idx = min(idx, self._last_proj_idx + 5)

        self._last_proj_idx = idx

        s_cur = self._s_pmm[idx]
        # zusätzlicher Schutz: s darf nie zurückgehen
        if not hasattr(self, "_last_s_cur"):
            self._last_s_cur = s_cur
        s_cur = max(s_cur, self._last_s_cur)
        self._last_s_cur = s_cur
    ...
    def _generate_gate_waypoints(
        ...
        short_after = 0.1         # 0.1 m hinter dem Gate
