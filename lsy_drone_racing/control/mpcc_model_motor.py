...
def symbolic_dynamics_euler_mpcc(
    ...
    *,
    ...
    """
    # States and Inputs
    X = cs.vertcat(symbols.pos, symbols.rpy, symbols.vel, symbols.drpy, symbols.progress)
    if model_rotor_vel:
        X = cs.vertcat(X, symbols.rotor_vel)
    ...
    vtheta_dot = dvtheta_cmd

    if model_rotor_vel:
        X_dot = cs.vertcat(pos_dot, symbols.drpy, vel_dot, ddrpy, rotor_vel_dot, theta_dot, vtheta_dot)
    else:
        X_dot = cs.vertcat(pos_dot, symbols.drpy, vel_dot, ddrpy, theta_dot, vtheta_dot)
    Y = cs.vertcat(symbols.pos, symbols.rpy)
