...
- We use Global-Box-Constraints lbx and ubx for state constraints and soft constraints for position states.
- Dynamically set corridor-bounds per stage based on distance to next gate.
"""
...
def create_ocp_solver(
    ...
    # Alle States bekommen prinzipiell Bounds
    ...
    # Soft Constraints nur auf Position (x,y,z)
    ocp.constraints.idxsbx = np.array([0, 1, 2], dtype=int)
    nsbx = ocp.constraints.idxsbx.shape[0]

    ocp.cost.Zl = 5000 * np.diag(np.ones((nsbx,)))
    ocp.cost.Zu = 5000 * np.diag(np.ones((nsbx,)))
    ocp.cost.zl = np.zeros((nsbx,))
    ocp.cost.zu = np.zeros((nsbx,))
    ocp.constraints.soft_constraint = "SLACK"
...
class AttitudeMPC(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        # SOFT corridor sizes for soft constraints
    ...
    def compute_control(
        ...
        for k in range(self._N):
            ...
            # Corridor soft constraints for position states

            gate_pos = self.gates[self.current_gate_idx]  # (3,)

        

            # Only distance in x-direction (gate normal)
            dist_x_ref = abs(gate_pos[0] - pos_des[0])
            dist_x_now = abs(gate_pos[0] - obs["pos"][0])
            dist_z_now = abs(gate_pos[2] - obs["pos"][2])

            if dist_x_ref < self.gate_influence_radius and dist_x_now < 2*self.gate_influence_radius and dist_z_now < 2*self.gate_influence_radius:
            ...
            # bounds from OCP (rpy bounds etc.)
            lbx_k = self._ocp.constraints.lbx.copy()
            ubx_k = self._ocp.constraints.ubx.copy()

            # Position [0:3] tight around desired position
            lbx_k[0:3] = pos_des - corridor
            ubx_k[0:3] = pos_des + corridor
                # rpy-Bounds [3:6] bleiben wie in lbx/ubx aus dem OCP gesetzt

            # Stage im acados-Solver:
            #  - Stage 0 = initial state (x0 == obs), die setzen wir weiter oben
            #  - hier beschränken wir die Stages 1..N-1
            stage = k + 1
            if stage <= self._N - 1:
                self._acados_ocp_solver.set(stage, "lbx", lbx_k)
                self._acados_ocp_solver.set(stage, "ubx", ubx_k)
                # enable softening for the idxsbx (positions 0,1,2) via sl/su
                nsbx = self._ocp.constraints.idxsbx.shape[0]
                sl_k = np.zeros(nsbx)  # allow lower violations
                su_k = np.zeros(nsbx)  # allow upper violations
                self._acados_ocp_solver.set(stage, "sl", sl_k)
                self._acados_ocp_solver.set(stage, "su", su_k)
