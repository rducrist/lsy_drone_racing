...
- We use Global-Box-Constraints lbx and ubx for state constraints.
- A soft corridor around the reference trajectory is implemented as nonlinear path constraint h(x,p) with slacks.
"""
...
from casadi import MX, vertcat  # <-- NEU: für con_h_expr / Parameter
...
def create_acados_model(parameters: dict) -> AcadosModel:
    ...
    #Parameter & Path-Constraint h(x,p) ----------
    # p = [c_x, c_y, c_z, w_x, w_y, w_z]
    p = MX.sym("p", 6)
    model.p = p

    # Position from State ( X[0:3] = pos)
    px, py, pz = X[0], X[1], X[2]

    c_x, c_y, c_z = p[0], p[1], p[2]
    w_x, w_y, w_z = p[3], p[4], p[5]

    # normalized distance to corridor center
    h_x = (px - c_x) / w_x
    h_y = (py - c_y) / w_y
    h_z = (pz - c_z) / w_z

    # h(x,p) in acados
    model.con_h_expr = vertcat(h_x, h_y, h_z)
...
def create_ocp_solver(
    ...
    # Global state box constraints lbx, ubx
    ocp.constraints.idxbx = np.arange(nx, dtype=int)

    lbx = np.full(nx, -1e3)
    ubx = np.full(nx,  1e3)

    # Grobe Position-Bounds
    lbx[0:3] = -0.5
    ubx[0:3] =  0.5
    ...
    # Input Constraints (rpy < 30°)
    ocp.constraints.lbu = np.array(
        [-0.5, -0.5, -0.5, parameters["thrust_min"] * 4]
    )
    ocp.constraints.ubu = np.array(
        [0.5, 0.5, 0.5, parameters["thrust_max"] * 4]
    )
    ocp.constraints.idxbu = np.array([0, 1, 2, 3])

    # Path-Constraints h(x,p) with Slacks ----------
    # h(x,p) has dimension 3 (x,y,z-corridor)
    nh = 3
    # |h_i| <= 1  -> corridor (center & width come from p)
    ocp.constraints.lh = -np.ones((nh,))
    ocp.constraints.uh =  np.ones((nh,))

    # We make ALL h-components soft: Slacks sh
    ocp.constraints.idxsh = np.arange(nh, dtype=int)
    nsh = nh

    # Slack-Bounds (>=0, Startwert 0)
    ocp.constraints.lsh = np.zeros((nsh,))
    ocp.constraints.ush = np.zeros((nsh,))

    # Costs on Slacks (soft constraints)
    ocp.cost.Zl = 5000 * np.eye(nsh)
    ocp.cost.Zu = 5000 * np.eye(nsh)
...
class AttitudeMPC(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        self.gate_radius = 0.2
        self.current_gate_idx = 0

        # SOFT corridor sizes for soft constraints (Halbbreiten)
        self.corridor_default = np.array([0.1, 0.1, 0.1])  # in m
        self.corridor = np.array([0.1, 0.1, 0.1])  # in m, near gate
        self.gate_influence_radius = 0.5  # when to switch to tight corridor
        ...
        # --- DEBUG: check that Zl/Zu made it into the solver ---
        Zl_solver = self._acados_ocp_solver.cost_get(0, "Zl")
        Zu_solver = self._acados_ocp_solver.cost_get(0, "Zu")
        print("Zl in solver:", Zl_solver)
        print("Zu in solver:", Zu_solver)
    ...
    def compute_control(
        ...
        # Build horizon references + Parameter für h(x,p)
        for k in range(self._N):
            ...
            # Parameter p_k für Path-Constraint h(x,p) ----
            gate_pos = self.gates[self.current_gate_idx]  # (3,)

            # Nur x-/z-Entfernung zur Gate-Ebene für Korridorwahl
            dist_x_ref = abs(gate_pos[0] - pos_des[0])
            dist_x_now = abs(gate_pos[0] - obs["pos"][0])
            dist_z_now = abs(gate_pos[2] - obs["pos"][2])

            if (
                dist_x_ref < self.gate_influence_radius
                and dist_x_now < 2 * self.gate_influence_radius
                and dist_z_now < 2 * self.gate_influence_radius
            ):
                corridor = self.corridor
            else:
                corridor = self.corridor_default

            # Korridorzentrum = Referenzposition (wie vorher über lbx/ubx)
            center = pos_des
            p_k = np.concatenate([center, corridor])
            last_p_k = p_k

            # Parameter für Stage k setzen (wirkt in h(x,p))
            self._acados_ocp_solver.set(k, "p", p_k)
        ...
        # Parameter p auch am Terminal setzen (z.B. letzter Korridor)
        if last_p_k is not None:
            self._acados_ocp_solver.set(self._N, "p", last_p_k)

        # Solve MPC
        self._acados_ocp_solver.solve()

        # Debug: Slacks der h-Constraints
        x1 = self._acados_ocp_solver.get(1, "x")
        sl = self._acados_ocp_solver.get(1, "sl")
        su = self._acados_ocp_solver.get(1, "su")
        print("x1 pos:", x1[:3])
        print("sl:", sl)
        print("su:", su)
