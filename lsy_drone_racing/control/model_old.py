...
import lsy_drone_racing.control.symbol as symbols
...
def symbolic_dynamics_euler_mpcc(
    model_rotor_vel: bool = True,
    *,
    mass: float,
    gravity_vec: Array,
    ...
    """
    # States and Inputs
    X = cs.vertcat(symbols.pos, symbols.rpy, symbols.vel, symbols.drpy, symbols.theta)
    if model_rotor_vel:
        X = cs.vertcat(symbols.pos, symbols.rpy, symbols.vel, symbols.drpy, symbols.rotor_vel, symbols.theta)
    U = symbols.cmd_rpyt_vth
    cmd_rpy = U[:3]
    cmd_thrust = U[3]
    vtheta = U[4]
    rot = rotation.cs_rpy2matrix(symbols.rpy)
    ...
    # MPCC progress dynamics
    theta_dot  = symbols.vtheta
    if model_rotor_vel:
        X_dot = cs.vertcat(pos_dot, symbols.drpy, vel_dot, ddrpy, rotor_vel_dot, theta_dot)
    else:
        X_dot = cs.vertcat(pos_dot, symbols.drpy, vel_dot, ddrpy, theta_dot)
    Y = cs.vertcat(symbols.pos, symbols.rpy)
    return X_dot, X, U, Y