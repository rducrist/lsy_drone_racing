...
from lsy_drone_racing.control.model_old import symbolic_dynamics_euler_mpcc
import casadi as cs
...
def create_acados_model(parameters: dict) -> AcadosModel:
    ...
    R_u   = parameters.get("R_inputs", 15.0) # weight for control inputs rpy
    R_T   = parameters.get("R_thrust", 15.0) # weight for thrust 
    ...
    stage_cost = (
        qc * ec_sq +            # contouring
        ql * el_sq +            # lag
        R_vth * vtheta_cmd**2 +# smooth progress accel
        R_u   * cs.dot(u_rpy, u_rpy) +  # input regularization
        R_T   * (u_T - 0.6)**2 -
        mu * vtheta_cmd           # progress reward
    )
...
def create_ocp_solver(
    ...
    # (rpy < 30°) and (thrust within physical limits) and (vtheta_cmd limits)

    ocp.constraints.lbu = np.array([-0.9, -0.9, -0.9, parameters["thrust_min"] * 4, 0.0])
    ocp.constraints.ubu = np.array([0.9, 0.9, 0.9, parameters["thrust_max"] * 4, 5.0])
    ...
    ocp.cost.Zu = 800 * np.ones((nsbx,))
    ocp.cost.zl = 1 * np.ones((nsbx,))
    ocp.cost.zu = 80 * np.ones((nsbx,))
