...
def _extract_torque(states, force: np.ndarray) -> np.ndarray:
    ...
    for attr in ("torque", "tau", "moments", "moment"):
        if hasattr(states, attr):
            return _to_numpy(getattr(states, attr))
...
def _snapshot_step(
    ...
    force = _to_numpy(states.force)
    torque = _extract_torque(states, force)
    return {
        "t": t,
        "action": actions.copy(),
        ...
        "terminated": terminated,
        "truncated": truncated,
        "disabled": _to_numpy(data.disabled_drones[0]),
        "target_gate": _to_numpy(obs["target_gate"]),
        "pos": _to_numpy(obs["pos"]),
        "quat": _to_numpy(obs["quat"]),
        "vel": _to_numpy(obs["vel"]),
        "ang_vel": _to_numpy(obs["ang_vel"]),
        "force": force[0].copy(),
